...
### paleta colorblind
pal <- scico(n = 8, categorical = T, palette = "navia")
```
...
+=======================================================+=======================================================================================+
| Se pueden estudiar grandes grupos poblacionales\      | No se tiene información del individuo,\                                               |
| \                                                     | por lo que no se puede ajustar por diferencias a nivel individual.                    |
...
g1 <- pos %>% ggplot(mapping = aes(x = X, y = Y)) +
  # puntos
    geom_point(color = pal[3], size = 2, alpha = .8) +
...
**Gráfico 1.** Edad y peso en 100 lactantes de sexo femenino.
...
**Gráfico 2.** Función lineal. Edad y peso en 100 lactantes de sexo femenino.
...
dat %>% 
  ggplot(mapping = aes(x = x, y = y)) +
  # geoms
  geom_point(color = pal[3], size = 4, alpha = .8) +
  geom_smooth(method = "lm", se = F, color = pal[6]) +
  geom_segment(mapping = aes(x = x, y = y, xend = x, yend = predict(fit)), 
               color = pal[3], linetype = "dashed") +
  # ejes
...
Para llevar a cabo el análisis en R y presentar las funciones y paquetes que nos pueden ayudar en la tarea vamos a trabajar con un set de datos llamado **cancer_rlm** que contiene información sobre la tasa de mortalidad por cáncer cada 100.000 habitantes de distintos condados de Estados Unidos.
...
La variable dependiente es la tasa de mortalidad (**target_death_rate**) y la independiente el % de cobertura pública de salud (**pct_public_coverage**).

### Diagrama de dispersión

Para dibujar gráficos de dispersión podemos utilizar funciones de ggplot2.
...
```{r}
cor(datos$pct_public_coverage, datos$target_death_rate,
    method = "pearson")
```
...
```{r}
cor.test(datos$pct_public_coverage, datos$target_death_rate)
```
...
```{r}
lm(target_death_rate ~ pct_public_coverage, data = datos)
```
...
```{r}
modelo <- lm(target_death_rate ~ pct_public_coverage, data = datos)
```
...
Los residuales o residuos para cada valor que surgen de la diferencia entre los valores predictivos calculados por el modelo y los valores reales.
...
Retomando la cuestión gráfica, podemos dibujar la recta de regresión lineal sobre el diagrama de dispersión hecho con `ggplot2` adicionando una capa más al gráfico mediante `geom_smooth()` e indicando "lm" como método. Además de la recta se puede ver el IC (zona gris alrededor de ella).
...
El residual o residuo de una estimación se define como la diferencia entre el valor observado y el valor calculado por el modelo de regresión.

A la hora de resumir el conjunto de residuales hay dos posibilidades:

-   La sumatoria del valor absoluto de cada residual.

-   La sumatoria del cuadrado de cada residual (RSS). Esta es la aproximación más empleada (mínimos cuadrados) ya que magnifica las desviaciones extremas.

En R vimos que estos residuales quedan almacenados dentro del objeto de regresión y pueden ser llamados mediante la expresión *nombre_del_objeto_de_regresion*`$residuals`
...
Un análisis visual de estos residuales se puede obtener fácilmente aplicando la función `plot()` al objeto de regresión modelado. La salida presentará 4 gráficas automáticas.
...
```{r}
check_model(modelo, check = c("normality","qq", "linearity",                       "homogeneity", "outliers"))
```

Además del análisis gráfico/visual de residuales se pueden aplicar test analíticos.
...
```{r}
# detectamos la observación mayor a 3 ds
rstudent(modelo)[rstudent(modelo) > 3]
```
...
En nuestro ejemplo, $R^2$ = `r r2(modelo)$R2` y $R^2$ ajustado = `r r2(modelo)$R2_adjusted`; por lo que podemos concluir que el modelo lineal simple no se ajusta demasiado bien a nuestros datos.

Esto indica que aproximadamente el 6,1 % de la variación en la tasa de mortalidad por cáncer se puede explicar por el modelo que solo contiene las proporciones de personas con acceso a la salud pública como variable explicativa. Es bajo, por lo que las predicciones de la ecuación de regresión son bastante poco confiables, ya que también significa que hay otro 93.9 % de la variación que aún no se explica, por lo que quizás agregar otras variables independientes podría mejorar el ajuste del modelo.
...
La tabla de analisis de varianza muestra el mismo resultado que el bloque final de `summary(modelo)` con un valor F de `r summary(modelo)$fstatistic[1] %>% round(.,1)` y un valor $p$ significativo \<0,001.

#### Resumen del resultado del ejemplo

Se llevó a cabo una regresión lineal simple para analizar la relación entre la proporción de personas con acceso al sistema de salud público y la tasa de mortalidad por cáncer en 236 condados de Estados Unidos.

El diagrama de dispersión mostró una fuerte relación lineal negativa entre los dos, que se confirmó con un coeficiente de correlación de Pearson de `r cor(datos$pct_public_coverage, datos$target_death_rate, method = "pearson") %>% round(., 2)` . La regresión lineal simple mostró una relación significativa entre las variables (t = 3,89 p \< 0,001).

El coeficiente de pendiente de la recta para la proporción de ciclistas fue de 0,73 por lo que la tasa de mortalidad por cáncer aumenta 0,7 % por cada 1 % que sube la proporción de personas que solo tienen acceso al sistema público de salud.

El valor $R^2$ mostró que el 5,7 % de la variación en la tasa de mortalidad por cáncer se puede explicar por el modelo que solo contiene acceso al sistema público de salud.
