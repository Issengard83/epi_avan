...
tibble(
  ...
  geom_point(color = pal[3], alpha = .7) +
...
En vez de la ecuación de la recta, ahora tenemos otra ecuación que expresa la variable repuesta ($Y$) en función de la/las variables independientes, en realidad expresa la probabilidad de que ocurra un hecho en función de ciertas variables que se presumen relevantes.
...
En R, las funciones `r2_mcfadden()`, `r2_coxsnell()`, `r2_nagelkerke()`, `r2_tjur()` pertenecientes al paquete `performance`, nos permite calcularlos.
...
-   *family:* hace referencia a la familia de distribuciones y, en link, a la función de enlace elegida para el ajuste de este modelo. Las opciones disponibles son `binomial`, `gaussian`, `Gamma`, `inverse.gaussian`, `poisson`, `quasi`, `quasibinomial` y `quasipoisson`.

    -   La familia `gaussian` tiene por defecto el enlace (link) `identity`, que equivale a ajustar un modelo de regresión lineal, pero también admite los enlaces`log` e `inverse`;

    -   La familia `binomial` tiene por defecto el enlace `logit`, admitiendo también los enlaces `probit`, `cauchit`, `log` y `cloglog`;

    -   La familia `poisson` tiene por defecto enlace `log`, permitiendo también los enlaces `identity` y `sqrt`;

    -   La familia `Gamma`, admite enlaces `inverse`, `identity` y `log`;

    -   La familia `inverse.gaussian` los enlaces `$1/{µ2}$`, `inverse`, `identity` y `log`

Estas posibilidades abarcan la mayor parte de las familias de GLMs existentes. Sin embargo, a lo largo de este curso nos centraremos en aquellas más utilizadas en el análisis de datos epidemiológicos: `gaussian` (Unidad 3), `binomial` (Unidad 4) y `poisson` (Unidad 5) con sus funciones de enlace canónicas (aquellas que se aplican por defecto a cada una de las distribuciones de errores).
...
También podemos modificar estos niveles de referencia mediante la función `relevel()` de `R base` o la función `fct_relevel()` de `tidyverse`, cuando estos se encuentren invertidos.
...
### Tidyverse
fct_relevel(datos$camina, "1", after = 0)
```
...
```{r}
performance_roc(mod6) %>% # calcula área bajo la curva
  plot() # muestra el gráfico
```

Si quisiéramos editar el gráfico de la curva ROC entonces debemos agregar el siguiente código:
...
Nuestro modelo logra un AUC de `r performance_accuracy(mod6)`.
...
Se puede presentar gráficamente la regresión usando los paquetes `broom` y `ggplot2` incluídos en `tidyverse`:
...
mod6_df %>% 
  ...
  geom_vline(xintercept = 1, linetype = "dashed", color = pal[6]) +
  geom_pointrange(color = pal[3]) +
  labs(x = "Estimación", y = NULL) +
  theme_minimal()
     
```

También podemos generar un gráfico similar usando los paquetes `parameters` y `see`:
...
compare_models(mod6a, mod6b, exponentiate = T)
```
...
compare_models(mod6, mod7, exponentiate = T)
```
...
Lüdecke, Patil, Ben-Shachar, Wiernik, Waggoner & Makowski. Visualisation Toolbox for 'easystats' and Extra Geoms, Themes and Color Palettes for 'ggplot2', 2020. CRAN. Available from <https://easystats.github.io/see/>
...
Michael Friendly. vcdExtra: 'vcd' Extensions and Additions, 2021. R package version 0.7-5. <https://CRAN.R-project.org/package=vcdExtra>

Xavier Robin, Natacha Turck, Alexandre Hainard, Natalia Tiberti, Frédérique Lisacek, Jean-Charles Sanchez and Markus Müller. pROC: an open-source package for R and S+ to analyze and compare ROC curves. BMC Bioinformatics, 12, p. 77. 2011 DOI: 10.1186/1471-2105-12-77 <http://www.biomedcentral.com/1471-2105/12/77/>
